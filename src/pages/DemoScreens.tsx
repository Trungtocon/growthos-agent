...
function RunConsoleRealPage() {
  ...
  return (
    <div data-demo-source={data.run.id}>
      ...
      <div data-parity-id="run.main-grid" className="mt-5 grid grid-cols-[1.05fr_1fr_360px] gap-4"><div data-parity-id="run.timeline-panel" className="min-w-0"><div data-parity-id="run.timeline-card"><Panel title="Live Timeline"><div className="space-y-3 p-4">{data.timelineRows.map((step, index) => <div key={step.name} className="grid grid-cols-[24px_1fr_80px_70px_70px_20px] items-center gap-3 rounded-lg border border-slate-100 p-3 text-sm"><span className="grid h-6 w-6 place-items-center rounded-full bg-blue-50 text-xs font-bold text-brand-600">{index + 1}</span><b>{step.name}</b><Badge tone={statusTone(step.status)}>{step.status}</Badge><span>{step.time}</span><span>{step.duration}</span><span>{step.cost}</span></div>)}</div></Panel></div></div><div data-parity-id="run.logs-panel" className="min-w-0 space-y-4"><div data-parity-id="run.tool-calls-card" className="h-[270px] overflow-hidden"><Panel title="Tool Calls" className="h-full overflow-hidden"><div className="p-4"><ToolCallList /></div></Panel></div><div data-parity-id="run.logs-card"><Panel title="Logs"><div className="relative"><pre className="m-4 h-72 overflow-hidden rounded-xl bg-slate-950 p-5 font-mono text-xs leading-6 text-slate-200">0001 [09:42:13] INFO Run started by Hermes QA Agent{'\n'}0002 [09:42:14] INFO Loading ticket context...{'\n'}0003 [09:42:16] INFO Loading skill: growthos-module-uat v1.2{'\n'}0004 [09:42:18] TOOL read_file - PROJECT_BIBLE.md{'\n'}0005 [09:42:21] TOOL read_file - module-3-architecture.md{'\n'}0006 [09:42:25] TOOL run_build - npm run build{'\n'}0007 [09:44:07] WARN analyze_route - /module-3/leads missing redirect{'\n'}0008 [09:44:29] INFO Generating QA report...{'\n'}0009 [09:44:31] INFO Collecting evidence and screenshots{'\n'}0010 [09:44:31] INFO Writing QA report...</pre></div></Panel></div></div><RunInspector runId={data.run.id} /></div>
    </div>
  );
}
