...
function RunInspector({ runId }: { runId: string }) {
  ...
  return <div data-parity-id="run.inspector-panel" className="space-y-4"><Panel title="Run Inspector"><SideRows rows={[['Agent', data.agent.name], ['Runtime', 'hermes_local'], ['Model', 'Claude Sonnet via OpenRouter'], ['Workspace', '/workspaces/growthos'], ['Enabled tools', enabledTools], ['Checkpoints', 'Enabled']]} /></Panel><div data-parity-id="run.cost-card"><Panel title="Chi phí"><SideRows rows={[['Estimated cost', '$0.08'], ['Actual cost', `$${data.run.cost.toFixed(3)}`], ['Budget status', 'Trong giới hạn']]} /></Panel></div><div data-parity-id="run.risk-card"><Panel title="Rủi ro"><SideRows rows={[['Risk level', workflowStatusLabel(data.run.riskLevel)], ['Policy checks', 'Passed'], ['Pending approvals', '0']]} /></Panel></div><div data-parity-id="run.artifacts-card"><Panel title={`Artifacts (${data.artifacts.length})`}><div className="divide-y divide-slate-100 p-4 text-sm">{data.artifacts.map((artifact) => <div key={artifact.id} className="flex justify-between py-3"><span>{artifact.name}</span><Download className="h-4 w-4 text-slate-400" /></div>)}</div></Panel></div><div data-parity-id="run.controls-card"><Panel title="Controls"><div className="grid grid-cols-2 gap-2 p-4"><Button data-workflow="run-pause" variant="warning" onClick={() => void pauseRun(runId)}>Pause run</Button><Button data-workflow="run-resume" variant="secondary" onClick={() => void resumeRun(runId)}>Resume run</Button><Button data-workflow="run-retry" variant="secondary" onClick={() => void retryRun(runId)}>Retry run</Button><Button data-workflow="run-cancel" variant="secondary" onClick={() => void cancelAgentRun(runId)}>Cancel run</Button></div></Panel></div></div>;
}
