...
function ExecutionGraphCompactWidget({ type, id }: { type: 'agent' | 'ticket' | 'run'; id: string }) {
  const graph = type === 'agent' ? selectExecutionGraphByAgent(id) : type === 'ticket' ? selectExecutionGraphByTicket(id) : selectExecutionGraphByRun(id);
  return (
...
function AgentDetailRealPage() {
  ...
  return (
    <div data-demo-source={data.agent.id}>
      ...
      <LearningRecommendationCompactWidget runId={DEMO_RUN_ID} surface="agent" />
      <RecommendationExecutionCompactWidget surface="agent" />
      <ImprovementLoopCompactWidget surface="agent" />
      ...
      <RuntimeCertificationCompactWidget surface="agent" />
...
function TicketSide() {
  ...
  const policyStatus = data.policyReport?.status ?? 'not evaluated';
  const policySummary = data.blockingReasons.length ? `blocked: ${data.blockingReasons[0]}` : data.planWarnings.length ? `warning: ${data.planWarnings[0]}` : policyStatus;
  const planSummary = plan ? `${plan.status} - policy ${policyStatus} - ${planCoverage} caps` : 'No plan';
  const estimatedCost = data.executionEstimate ? `$${data.executionEstimate.estimatedCost.toFixed(3)}` : '$0.000';
  const actualCost = data.usageLedger ? `$${data.usageLedger.actualTotal.toFixed(3)}` : '$0.000';
  const estimatedDuration = data.executionEstimate ? `${data.executionEstimate.estimatedDuration}s` : '-';
  const workflowSummary = data.workflowAnalytics ? `$${data.workflowAnalytics.totalCost.toFixed(3)} / ${Math.round(data.workflowAnalytics.totalDuration)}s` : estimatedDuration;
  const estimatedRisk = data.executionEstimate?.riskLevel ?? data.ticket.riskLevel;
  return (
    <div className="space-y-2">
      <div data-parity-id="ticket.status-card">
        <Panel title="Trang thai thuc thi">
          <SideRows rows={[
            ['Status', <WorkflowEntityStatus key="ticket-status" entityId={data.ticket.id} status={data.ticket.status} />],
            ['Runtime Mode', `${data.runtimeReadiness.mode} / ${data.runtimeReadiness.hermesStatus}`],
      ...
      <Panel title="Chi phi"><SideRows rows={[["Est / Actual", `${estimatedCost} / ${actualCost}`], ['Workflow', workflowSummary], ['Quota status', data.quotaStatus ?? 'ok']]} /></Panel>
      <Panel title="Rui ro"><SideRows rows={[["Risk level", workflowStatusLabel(estimatedRisk)], ['Policy checks', `${policyStatus} / ${data.authorization.governanceDecision.summary.latestDecision} / ${data.authorization.governanceEnforcement.summary.latestAction}`], ['Pending approvals', `${data.approval?.status === 'pending' ? '1' : '0'} / exec ${data.authorization.approvalExecution.summary.pending}`]]} /></Panel>
      ...
      <div data-parity-id="ticket.artifacts-card" className="h-[192px] overflow-hidden">
        <Panel title="Ket qua dau ra">
          <div className="space-y-3 p-4">
            <div className="flex items-center justify-between text-xs font-semibold text-slate-500">
              <span>{artifacts.length} linked artifacts - Tools {data.completedToolCalls.length}/{data.totalToolsExecuted || data.run?.toolCalls.length || 0} - Policy {policySummary} {data.latestStreamEvent ? `- ${data.latestStreamEvent.message}` : ''}</span>
              <div className="flex items-center gap-3">
                ...
                <button data-workflow="ticket-start-plan" disabled={Boolean(plan && !data.canStartPlan)} onClick={() => plan ? data.canStartPlan ? void startRunFromPlan(plan.id) : undefined : void createRunPlanAction(data.ticket.id, 'demo-run-execution')} className="font-semibold text-brand-600 disabled:text-slate-400">Start plan</button>
...
function SideRows({ rows, compact = false }: { rows: Array<[string, React.ReactNode]>; compact?: boolean }) {
  return <div className={`${compact ? 'space-y-2 p-3 text-xs' : 'space-y-3 p-4 text-sm'}`}>{rows.map(([a, b]) => <div key={a} className="flex justify-between gap-3"><span>{a}</span><b className="text-right">{b}</b></div>)}</div>;
}
...
function RunConsoleRealPage() {
  ...
  return (
    <div data-demo-source={data.run.id}>
      ...
      <RunEvaluationCompactWidget runId={data.run.id} surface="run" />
      <EvaluationFeedbackCompactWidget runId={data.run.id} surface="run" />
      <FeedbackActionPlanCompactWidget runId={data.run.id} surface="run" />
      <ActionPlanExecutionCompactWidget runId={data.run.id} surface="run" />
      <ImprovementOutcomeCompactWidget runId={data.run.id} surface="run" />
      <LearningRecommendationCompactWidget runId={data.run.id} surface="run" />
      <RecommendationExecutionCompactWidget surface="run" />
      <ImprovementLoopCompactWidget surface="run" />
      ...
      <WorkerRecoveryCompactWidget surface="run" />
      <RuntimeCertificationCompactWidget surface="run" />
      <CertifiedSandboxRunCompactWidget surface="run" />
      <ProductionReadinessCompactWidget surface="run" />
...
function RunInspector({ runId }: { runId: string }) {
  const { data } = useRunConsoleData();
  const readiness = selectGovernanceReadinessReport();
  const discovery = data.hermesDiscovery;
  const registry = data.toolRegistry;
  const compatibleModels = registry.compatibility.filter((item) => item.supported).length;
  const primaryTool = registry.tools.find((tool) => tool.enabled);
  const plan = data.currentPlan;
  const missingCapabilities = data.missingCapabilities.length ? data.missingCapabilities.join(', ') : 'None';
  const estimate = data.executionEstimate;
  const estimateSummary = estimate ? `$${estimate.estimatedCost.toFixed(3)} - ${estimate.estimatedDuration}s - ${estimate.riskLevel}` : 'No estimate';
  const ledger = data.usageLedger;
  const ledgerEstimate = ledger?.estimatedTotal ?? estimate?.estimatedCost ?? 0;
  const ledgerActual = ledger?.actualTotal ?? data.run.cost;
  const runVariance = data.runReconciliation ?? data.reconciliationReport.records[0];
  const planPolicyStatus = data.policyReport?.status ?? 'not evaluated';
  const planStatus = plan ? `${plan.status} - policy ${planPolicyStatus} - ${plan.steps.length} steps - ${estimateSummary}` : 'No plan';
  return <div data-parity-id="run.inspector-panel" className="space-y-4"><div className="h-[262px] overflow-hidden"><Panel title="Run Inspector"><SideRows compact rows={[["Tool Registry", <span key="runtime-discovery" className="inline-flex items-baseline gap-2 leading-none"><span>{data.runtimeReadiness.mode} / {data.runtimeReadiness.hermesStatus} / {registry.tools.length} tools</span><button data-runtime-discovery-refresh onClick={() => void refreshRuntimeDiscovery()} className="align-baseline text-xs font-bold leading-none text-brand-600">Refresh</button></span>], ['Available Tools', `${primaryTool?.name ?? 'Mock runtime tools'} / source ${data.runtimeReadiness.currentRunSource ?? 'mock'}`], ['Capabilities', `${data.capabilities.length} mapped`], ['Workflow Readiness', data.workflowReadiness.ready ? 'Ready' : `Missing ${data.workflowReadiness.missingCapabilities.length}`], ['Missing Capabilities', plan?.missingCapabilities.join(', ') || missingCapabilities], ['Compatible Models', `${compatibleModels} mappings / sync ${(data.runtimeReadiness.lastSandboxSync ?? discovery.checkedAt).slice(0, 16).replace('T', ' ')}`]]} /></Panel></div><div data-parity-id="run.cost-card"><Panel title="Cost Reconciliation"><SideRows rows={[["Est / Actual / Provider", `$${(runVariance?.estimatedCost ?? ledgerEstimate).toFixed(3)} / $${(runVariance?.actualCost ?? ledgerActual).toFixed(3)} / $${(runVariance?.providerCost ?? data.providerCost).toFixed(3)}`], ['Variance', `$${(runVariance?.variance ?? data.costVariance.variance).toFixed(3)} - ${runVariance?.variancePercent ?? data.costVariance.variancePercent}%`], ['Plan Status', `${planStatus} - ${data.varianceSeverity}`]]} /></Panel></div><div data-parity-id="run.risk-card"><Panel title="Rui ro"><SideRows rows={[["Risk level", workflowStatusLabel(data.run.riskLevel)], ['Policy checks', `Passed / ${data.authorization.governanceDecision.summary.latestDecision} / ${data.authorization.governanceEnforcement.summary.latestAction}`], ['Pending approvals', `${data.authorization.approvalExecution.summary.pending} exec / denied ${data.authorization.deniedActionSummary.totalDenied}`]]} /><span className="sr-only">Runtime Mode {data.runtimeReadiness.mode}; requested {data.runtimeReadiness.requestedMode}; Enterprise readiness {readiness.status}; blocked reasons {readiness.blockedReasons.length}; Hermes message {data.runtimeReadiness.hermesMessage}; fallback {data.runtimeReadiness.fallbackReason}</span></Panel></div><div data-parity-id="run.artifacts-card" className="h-[216px] overflow-hidden"><Panel title={`Artifacts (${data.artifacts.length})`}><div className="p-3"><ArtifactViewer artifacts={data.artifacts} selectedArtifactId={data.selectedArtifactId} preview={data.selectedArtifactPreview ?? data.primaryArtifactPreview} onSelectArtifact={selectArtifact} compact /></div></Panel></div><div data-parity-id="run.controls-card"><Panel title="Controls"><div className="grid grid-cols-2 gap-2 p-4"><Button data-workflow="run-pause" variant="warning" onClick={() => void pauseRun(runId)}>Pause run</Button><Button data-workflow="run-resume" variant="secondary" onClick={() => void resumeRun(runId)}>Resume run</Button><Button data-workflow="run-retry" variant="secondary" onClick={() => void retryRun(runId)}>Retry run</Button><Button data-workflow="run-cancel" variant="secondary" onClick={() => void cancelAgentRun(runId)}>Cancel run</Button></div></Panel></div></div>;
}
function ApprovalCenterRealPageLegacy() {
  ...
  const executionRequest = data.approvalExecution.pending[0] ?? data.approvalExecution.latestRequest;
  return (
...
function ApprovalCenterRealPage() {
  ...
  const selectedEvents = workflow.events.filter((event) => event.entityId === selected.id);
  const executionRequest = data.approvalExecution.pending[0] ?? data.approvalExecution.latestRequest;
  const executionStatus = executionRequest ? `${executionRequest.runtimeAction} / ${executionRequest.status}` : 'No active hold';
  return (
    <div data-demo-source={`approvals:${data.rawApprovals.length}:exec-${data.approvalExecution.summary.totalRequests}`}>
      <div data-parity-id="approval.header">
        <PageHeader title="Approval Center" subtitle="Phê duyệt các hành động rủi ro trước khi AI Agent tiếp tục thực thi" actions={<><Button variant="secondary"><ShieldCheck className="h-4 w-4" />Approval Policy</Button><Button variant="secondary"><Download className="h-4 w-4" />Export Approval Log</Button><Button><Layers3 className="h-4 w-4" />Bulk Review</Button></>} />
      </div>
      <div data-parity-id="approval.kpi-band">{kpiGrid(mergeKpiData(approvalKpis, data.kpis))}</div>
      <div className="mt-5 flex flex-wrap gap-2">{['All 14', 'High Risk 5', 'Terminal 4', 'File Changes 3', 'Database 1', 'Email 2', 'Budget 1', 'MCP 1', 'Overdue 3'].map((filter, index) => <button key={filter} data-interaction={`approval-filter-${filter}`} onClick={() => { setRouteFilter('/approvals', 'risk', filter === 'High Risk 5' ? 'High' : 'All'); setSearchQuery(filter === 'Terminal 4' ? 'Hermes' : ''); }} className={`rounded-lg border px-4 py-2 text-sm font-semibold ${index === 0 ? 'border-brand-500 bg-blue-50 text-brand-700' : 'border-slate-200 bg-white text-slate-600'}`}>{filter}</button>)}</div>
      <div data-parity-id="approval.main-grid" className="mt-4 grid h-[1424px] grid-cols-[0.82fr_1.08fr] gap-4 overflow-hidden">
        <div data-parity-id="approval.queue-panel" className="h-full overflow-hidden">
          <div data-parity-id="approval.queue-card" className="h-full overflow-hidden">
            <Panel title={`Danh sách chờ phê duyệt (${14 + data.approvalExecution.summary.pending})`} action={<><button className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold">Mới nhất</button><Button variant="secondary" aria-label="Filter approvals"><ListFilter className="h-4 w-4" /></Button></>}>
              <div className="space-y-3 p-3">
                {data.approvals.map((approval, index) => <div key={approval.title} data-interaction="select-approval" onClick={() => selectApproval(approval.id)} className={`rounded-xl border p-4 ${approval.id === selected.id ? 'border-brand-500 ring-2 ring-blue-100' : 'border-slate-200'}`}><div className="grid grid-cols-[44px_1fr_180px_82px_26px] gap-3"><div className="grid h-11 w-11 place-items-center rounded-lg bg-slate-800 text-white">{approval.id === selected.id ? '>' : index + 1}</div><div><h3 className="font-bold">{approval.title}</h3><div className="mt-3 grid grid-cols-2 gap-2 text-sm text-slate-600"><span>Agent · {approval.agent}</span><span>Ticket · {approval.ticket}</span><span>Tool · {approval.originatingTool ?? approval.project}</span><span>Requested · {approval.requested}</span></div></div><div className="space-y-2 text-sm"><div className="flex justify-between"><span>Risk</span><Badge tone={statusTone(approval.risk)}>{approval.risk}</Badge></div><div className="flex justify-between"><span>Impact</span><b>{approval.impact}</b></div></div><Button variant="secondary">Review</Button><MoreButton /></div></div>)}
              </div>
            </Panel>
          </div>
        </div>
        <div data-parity-id="approval.detail-panel" className="h-full space-y-4 overflow-hidden">
          <div data-parity-id="approval.detail-card">
            <Panel>
              <div className="p-5">
                <div className="mb-5 flex items-start justify-between"><div className="flex items-center gap-4"><div className="grid h-12 w-12 place-items-center rounded-lg bg-slate-800 text-white text-2xl">&gt;</div><div><h2 className="text-xl font-bold">{selected.title}</h2><div className="mt-2 flex gap-4 text-sm"><WorkflowEntityStatus entityId={selected.id} status={selected.status} /><span>ID: APPR-2381</span></div></div></div></div>
                <div className="grid grid-cols-[170px_1fr] gap-4 rounded-xl border border-slate-200 p-4 text-sm">{[['Agent', selected.agent], ['Runtime', `${data.runtimeReadiness.mode} / Hermes ${data.runtimeReadiness.hermesStatus} / source ${data.runtimeReadiness.currentRunSource ?? 'mock'}`], ['Liên quan ticket', selected.ticket], ['Execution hold', executionStatus], ['Tool', selected.originatingTool ?? 'ProduceArtifact'], ['Lý do', executionRequest?.reasons[0] ?? 'Cần chạy test để xác minh module trước khi tạo QA report.'], ['Risk level', selected.risk], ['Resume status', data.approvalExecution.summary.latestStatus], ['Artifact', data.selectedArtifactPreview?.name ?? 'Paperclip_QA_Runtime_Packet.md']].flatMap(([a, b]) => [<span key={`${a}-label`} className="text-slate-500">{a}</span>, <b key={`${a}-value`} className="text-slate-800">{b}</b>])}</div>
                <div data-parity-id="approval.policy-panel" className="mt-5 h-[150px] overflow-hidden rounded-xl border border-blue-200 bg-blue-50 p-4"><h3 className="font-bold text-brand-700">Gợi ý từ AI</h3><p className="mt-2 text-sm leading-6 text-slate-700">{executionRequest ? `Approval execution đang giữ ${executionRequest.runtimeAction}. Reviewer có thể approve để resume hoặc reject để cancel runtime.` : data.selectedArtifactPreview ? `Artifact liên quan: ${data.selectedArtifactPreview.contentSummary}` : 'Yêu cầu này có rủi ro trung bình vì chỉ chạy test trong local workspace. Có thể approve once, nhưng không nên tạo policy tự động cho mọi terminal command.'}</p><div className="mt-3 flex items-center gap-3"><ProgressBar value={92} /><b>92%</b></div></div>
                <div data-parity-id="approval.actions-card" className="mt-5 grid grid-cols-5 gap-3"><Button data-workflow="approval-approve" variant="success" onClick={() => void approveApproval(selected.id)}>Approve once</Button><Button data-workflow="approval-execution-approve" onClick={() => executionRequest ? void approveApprovalExecution(executionRequest.id) : void approveApproval(selected.id)}>Approve execution</Button><Button data-workflow="approval-reject" variant="danger" onClick={() => void rejectApproval(selected.id)}>Reject</Button><Button variant="warning" data-workflow="approval-request-changes" onClick={() => executionRequest ? void requestApprovalExecutionChanges(executionRequest.id) : undefined}>Request changes</Button><Button variant="secondary" data-workflow="approval-escalate" onClick={() => executionRequest ? void escalateApprovalExecution(executionRequest.id) : undefined}>Escalate</Button><div className="col-span-5"><WorkflowInlineError entityId={selected.id} /></div></div>
              </div>
            </Panel>
          </div>
          <div data-parity-id="approval.audit-card" className="h-[102px] overflow-hidden"><Panel title="Lịch sử phê duyệt" className="h-full overflow-hidden"><div className="space-y-3 p-4 text-sm"><div>{data.approvalExecution.latestEvent?.message ?? 'Hermes QA Agent tạo yêu cầu phê duyệt · 8 phút trước'}</div><WorkflowTimeline events={selectedEvents} /></div></Panel></div>
        </div>
      </div>
    </div>
  );
