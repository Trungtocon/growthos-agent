...
function TicketSide() {
  ...
  const variance = data.estimatedVsActualCost ? `$${data.estimatedVsActualCost.variance.toFixed(3)}` : '$0.000';
  ...
  return (
    <div className="space-y-2">
      ...
      <Panel title="Chi phÃ­"><SideRows rows={[['Est / Actual', `${estimatedCost} / ${actualCost}`], ['Workflow', workflowSummary], ['Quota status', data.quotaStatus ?? 'ok']]} /></Panel>
      <Panel title="Rá»§i ro"><SideRows rows={[['Risk level', workflowStatusLabel(estimatedRisk)], ['Policy checks', policyStatus], ['Pending approvals', data.approval?.status === 'pending' ? '1' : '0']]} /></Panel>
...
function RunInspector({ runId }: { runId: string }) {
  ...
  const ledgerVariance = data.estimatedVsActualCost?.variance ?? 0;
  const quotaSummary = data.quotaWarnings.length ? data.quotaWarnings[0] : data.quotaStatus;
  const workspace = data.workspaceAnalytics;
  const topTool = data.topTools[0]?.toolId ?? 'none';
