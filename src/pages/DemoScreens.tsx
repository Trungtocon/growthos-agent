      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
        ...
        <div data-parity-id="org.main-grid" className="grid grid-cols-[1fr_280px] gap-4">
          ...
          <div data-parity-id="org.detail-panel">
            <Panel><div data-parity-id="org.agent-card" className="h-[328px] overflow-hidden p-4"><div className="mb-4 flex items-center gap-3"><AvatarBot tone="purple" label="AI" /><div className="min-w-0 flex-1"><div className="truncate font-bold">{selectedOrgAgent.name}</div><div className="text-sm text-slate-500">{selectedOrgAgent.role}</div></div><Badge tone="green">Running</Badge></div><MetricRows rows={[['Tải công việc', selectedOrgAgent.currentTicketIds.length * 18 + 40], ['Hiệu suất', selectedOrgAgent.healthScore], ['Tin cậy', selectedOrgAgent.trustScore]]} /><div className="mt-3 grid grid-cols-[96px_1fr] gap-y-2 text-xs"><span className="text-slate-500">Manager</span><b>CTO Agent</b><span className="text-slate-500">Task hiện tại</span><b>Audit Module 3</b><span className="text-slate-500">Chi phí tháng</span><b>$420</b></div><div className="mt-3 flex flex-wrap gap-1">{['ui-parity-audit', 'File', 'Terminal', 'Browser'].map((item) => <Badge key={item} tone="blue">{item}</Badge>)}</div><div className="mt-3 grid gap-2"><Button>View Agent Detail</Button><Button variant="secondary">Assign Task</Button><Button variant="secondary">Pause Agent</Button></div></div></Panel>
            <div data-parity-id="org.insight-card" className="mt-4 h-[110px] overflow-hidden rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
              <div className="font-bold text-slate-900">Gợi ý từ AI</div>
              <div className="mt-2 space-y-1.5 text-xs">
                {['Marketing quá tải 82%', 'Thêm Video Script Agent', 'Hermes QA hiệu suất cao'].map((item) => <div key={item} className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-2 py-1"><span>{item}</span><RowAction /></div>)}
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div data-demo-source={`approvals:${data.rawApprovals.length}`}>
      ...
      <div data-parity-id="approval.main-grid" className="mt-4 grid h-[1424px] grid-cols-[0.82fr_1.08fr] gap-4 overflow-hidden">
        <div data-parity-id="approval.queue-panel" className="h-full overflow-hidden"><div data-parity-id="approval.queue-card" className="h-full overflow-hidden"><Panel title="Danh sách chờ phê duyệt (14)" action={<><button className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold">Mới nhất</button><Button variant="secondary" aria-label="Filter approvals"><ListFilter className="h-4 w-4" /></Button></>}><div className="space-y-3 p-3">{data.approvals.map((approval, index) => <div key={approval.title} data-interaction="select-approval" onClick={() => selectApproval(approval.id)} className={`rounded-xl border p-4 ${approval.id === selected.id ? 'border-brand-500 ring-2 ring-blue-100' : 'border-slate-200'}`}><div className="grid grid-cols-[44px_1fr_180px_82px_26px] gap-3"><div className="grid h-11 w-11 place-items-center rounded-lg bg-slate-800 text-white">{approval.id === selected.id ? '>' : index + 1}</div><div><h3 className="font-bold">{approval.title}</h3><div className="mt-3 grid grid-cols-2 gap-2 text-sm text-slate-600"><span>Agent · {approval.agent}</span><span>Ticket · {approval.ticket}</span><span>Project · {approval.project}</span><span>Requested · {approval.requested}</span></div></div><div className="space-y-2 text-sm"><div className="flex justify-between"><span>Risk</span><Badge tone={statusTone(approval.risk)}>{approval.risk}</Badge></div><div className="flex justify-between"><span>Impact</span><b>{approval.impact}</b></div></div><Button variant="secondary">Review</Button><MoreButton /></div></div>)}</div></Panel></div></div>
