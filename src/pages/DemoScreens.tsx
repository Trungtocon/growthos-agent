...
function WorkforceOverviewParityPage() {
  return (
    <img className="h-[857px] w-[1414px]" src="/stitch_ui/parity_20/content.png" alt="AI Workforce overview content" />
  );
}

function OrgChartParityPage() {
  return (
    <img className="h-[873px] w-[1464px]" src="/stitch_ui/parity_21/content.png" alt="Org Chart view content" />
  );
}
...
function TicketsBoardParityPage() {
  return (
    <img className="h-[869px] w-[1462px]" src="/stitch_ui/parity_30/content.png" alt="Tickets Board content" />
  );
}

function TicketDetailParityPage() {
  return (
    <img className="h-[974px] w-[1334px]" src="/stitch_ui/parity_32/content.png" alt="Ticket Detail content" />
  );
}
...
function TicketDetail() {
  return (
    <div>
      ...
      <div className="grid grid-cols-[330px_1fr_410px] gap-4"><TicketInfo /><TranscriptPanel /><TicketSide /></div>
    </div>
  );
}

function TicketInfo() {
  return <div className="space-y-4"><Panel title="Thông tin ticket"><div className="space-y-4 p-4 text-sm">{[['Project', 'GrowthOS V2'], ['Linked goal', 'Hoàn thiện GrowthOS V2 đúng UI parity và nghiệp vụ'], ['Assignee', 'Hermes QA Agent'], ['Status', 'Running'], ['Priority', 'Cao'], ['Risk level', 'Trung bình'], ['Due date', 'Hôm nay'], ['Created by', 'Lê Tuấn Anh']].map(([a, b]) => <div key={a} className="grid grid-cols-[110px_1fr] gap-2"><span className="text-slate-500">{a}</span><span className="font-semibold text-slate-800">{b}</span></div>)}</div></Panel><Panel title="Mô tả"><p className="p-4 text-sm leading-6 text-slate-600">Kiểm tra Module 3 theo kiến trúc: Elementor tạo landing page, FluentForm tạo form, lead cần đổ về CRM đúng logic.</p></Panel><Panel title="Tiêu chí nghiệm thu"><div className="space-y-3 p-4 text-sm">{['Có verdict Pass / Conditional Pass / Fail', 'Có danh sách lỗi Critical / Major / Minor', 'Có evidence', 'Có next action'].map((item) => <label key={item} className="flex gap-2"><input type="checkbox" checked readOnly />{item}</label>)}</div></Panel></div>;
}

function TranscriptPanel() {
  return <Panel title="Run Transcript" action={<Button variant="secondary"><Filter className="h-4 w-4" />Filter</Button>}><div className="space-y-5 p-5"><div className="grid grid-cols-[80px_1fr] gap-4 text-sm"><span className="text-slate-500">09:14:22</span><div><b>Manager instruction</b><p className="mt-2 text-slate-600">Hãy audit Module 3 theo checklist growthos-module-uat.</p></div></div><div className="grid grid-cols-[80px_1fr] gap-4 text-sm"><span className="text-slate-500">09:14:28</span><div><b>Hermes QA Agent</b><p className="mt-2 text-slate-600">Tôi sẽ kiểm tra tài liệu kiến trúc, route, data flow và evidence.</p></div></div><ToolCallList /><div className="rounded-xl border border-slate-200 p-4 text-sm text-slate-500">Nhập phản hồi hoặc yêu cầu agent làm tiếp...</div><div className="flex justify-end gap-3"><Button variant="secondary">Yêu cầu tiếp tục</Button><Button variant="secondary">Tạo ticket follow-up</Button><Button><Send className="h-4 w-4" />Gửi</Button></div></div></Panel>;
}
...
function TicketSide() {
  return <div className="space-y-4"><Panel title="Trạng thái thực thi"><SideRows rows={[['Status', 'Running'], ['Elapsed time', '8m 24s'], ['Progress', '68%'], ['Current step', 'Generating QA report'], ['Last update', '1 phút trước']]} /><div className="px-4 pb-4"><ProgressBar value={68} /></div></Panel><Panel title="Chi phí"><SideRows rows={[['Estimated cost', '$0.08'], ['Actual cost', '$0.029'], ['Budget status', 'Trong giới hạn']]} /></Panel><Panel title="Rủi ro"><SideRows rows={[['Risk level', 'Trung bình'], ['Policy checks', 'Passed'], ['Pending approvals', '0']]} /></Panel><Panel title="Phê duyệt"><div className="grid grid-cols-2 gap-3 p-4"><Button variant="success">Approve</Button><Button variant="danger">Reject</Button><Button variant="warning">Request changes</Button><Button variant="secondary">Ask agent</Button></div></Panel></div>;
}
...
function RunConsole() {
  return (
    <div>
      ...
      <div className="mt-5 grid grid-cols-[1.05fr_1fr_360px] gap-4"><Panel title="Live Timeline"><div className="space-y-3 p-4">{runSteps.map(([name, status, time, duration, cost], index) => <div key={String(name)} className="grid grid-cols-[24px_1fr_80px_70px_70px_20px] items-center gap-3 rounded-lg border border-slate-100 p-3 text-sm"><span className="grid h-6 w-6 place-items-center rounded-full bg-blue-50 text-xs font-bold text-brand-600">{index + 1}</span><b>{name}</b><Badge tone={statusTone(String(status))}>{status}</Badge><span>{time}</span><span>{duration}</span><span>{cost}</span></div>)}</div></Panel><div className="space-y-4"><Panel title="Tool Calls"><div className="p-4"><ToolCallList /></div></Panel><Panel title="Logs"><pre className="m-4 h-72 overflow-hidden rounded-xl bg-slate-950 p-5 font-mono text-xs leading-6 text-slate-200">0001 [09:42:13] INFO Run started by Hermes QA Agent{'\n'}0002 [09:42:18] TOOL read_file - PROJECT_BIBLE.md{'\n'}0003 [09:42:25] TOOL run_build - npm run build{'\n'}0004 [09:44:07] WARN analyze_route - missing redirect{'\n'}0005 [09:44:29] INFO Generating QA report...</pre></Panel></div><RunInspector /></div>
    </div>
  );
}
...
function RunInspector() {
  return <div className="space-y-4"><Panel title="Run Inspector"><SideRows rows={[['Agent', 'Hermes QA Agent'], ['Runtime', 'hermes_local'], ['Model', 'Claude Sonnet via OpenRouter'], ['Workspace', '/workspaces/growthos'], ['Enabled tools', 'File, Terminal, Browser, Web'], ['Checkpoints', 'Enabled']]} /></Panel><Panel title="Chi phí"><SideRows rows={[['Estimated cost', '$0.08'], ['Actual cost', '$0.029'], ['Budget status', 'Trong giới hạn']]} /></Panel><Panel title="Artifacts (3)"><div className="divide-y divide-slate-100 p-4 text-sm">{['QA_Report_Module3.md', 'Console_Log.txt', 'Screenshot_Evidence.zip'].map((file) => <div key={file} className="flex justify-between py-3"><span>{file}</span><Download className="h-4 w-4 text-slate-400" /></div>)}</div></Panel><Panel title="Controls"><div className="grid grid-cols-2 gap-2 p-4"><Button variant="warning">Pause run</Button><Button variant="danger">Stop run</Button><Button variant="secondary">Retry run</Button><Button variant="secondary">Continue checkpoint</Button></div></Panel></div>;
}
