      ...
      <div className="mt-5 grid grid-cols-[360px_1fr_420px] gap-5">
        ...
        <Panel title="Control Actions">
          <div className="grid grid-cols-2 gap-2 p-4 text-sm" data-worker-control-actions>
            <Button variant="secondary" disabled={!eligibility.canPause} data-disabled-reason={!eligibility.canPause ? 'Worker must be running before it can be paused.' : undefined} title={!eligibility.canPause ? 'Worker must be running before it can be paused.' : undefined} onClick={() => { requestWorkerPause('Pause from control center.'); reload(); }}><Pause className="h-4 w-4" />Pause</Button>
            <Button variant="secondary" disabled={!eligibility.canResume} data-disabled-reason={!eligibility.canResume ? 'Worker must be paused before it can resume.' : undefined} title={!eligibility.canResume ? 'Worker must be paused before it can resume.' : undefined} onClick={() => { requestWorkerResume('Resume from control center.'); reload(); }}><Play className="h-4 w-4" />Resume</Button>
            <Button variant="secondary" disabled={!eligibility.canStop} data-disabled-reason={!eligibility.canStop ? 'No active worker is available to stop.' : undefined} title={!eligibility.canStop ? 'No active worker is available to stop.' : undefined} onClick={() => { requestWorkerStop('Stop from control center.'); reload(); }}><Square className="h-4 w-4" />Stop</Button>
            <Button variant="secondary" disabled={!eligibility.canRetryNow} data-disabled-reason={!eligibility.canRetryNow ? 'Retry is available only after a retryable worker failure.' : undefined} title={!eligibility.canRetryNow ? 'Retry is available only after a retryable worker failure.' : undefined} onClick={() => { requestWorkerRetryNow('Retry now from control center.'); reload(); }}><RotateCcw className="h-4 w-4" />Retry</Button>
            <Button variant="secondary" disabled={!eligibility.canSkip} data-disabled-reason={!eligibility.canSkip ? 'Skip requires an active queue item.' : undefined} title={!eligibility.canSkip ? 'Skip requires an active queue item.' : undefined} onClick={() => { requestWorkerSkipItem('Skip requested from control center.'); reload(); }}><XCircle className="h-4 w-4" />Skip</Button>
            <Button variant="secondary" disabled={!eligibility.canRequeue} data-disabled-reason={!eligibility.canRequeue ? 'Requeue requires an active or failed queue item.' : undefined} title={!eligibility.canRequeue ? 'Requeue requires an active or failed queue item.' : undefined} onClick={() => { requestWorkerRequeueItem('Requeue requested from control center.'); reload(); }}><RotateCcw className="h-4 w-4" />Requeue</Button>
            <Button variant="secondary" disabled={!eligibility.canEscalateToApproval} data-disabled-reason={!eligibility.canEscalateToApproval ? 'Approval escalation requires an active queue item.' : undefined} title={!eligibility.canEscalateToApproval ? 'Approval escalation requires an active queue item.' : undefined} onClick={() => { requestWorkerEscalateToApproval('Approval escalation from control center.'); reload(); }}><ShieldCheck className="h-4 w-4" />Approval</Button>
